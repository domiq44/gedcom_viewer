...
class TestGedcomViewer(unittest.TestCase):
    ...
    def test_detail_panel_can_indent_gedcom_by_level(self):
        panel = DetailPanel(self.root, self.viewer.translator)
        panel.indent_var.set(True)
        panel.display_raw("0 @I1@ INDI\n1 NAME John /DOE/\n2 GIVN John\n0 TRLR")

        rendered = panel.text_area.get("1.0", tk.END)
        self.assertIn("0 @I1@ INDI", rendered)
        self.assertIn("  1 NAME John /DOE/", rendered)
        self.assertIn("    2 GIVN John", rendered)
        self.assertIn("0 TRLR", rendered)

    def test_detail_panel_can_copy_raw_gedcom_to_clipboard(self):
        panel = DetailPanel(self.root, self.viewer.translator)
        panel.indent_var.set(True)
        panel.display_raw("0 @I1@ INDI\n1 NAME John /DOE/\n0 TRLR")

        with patch.object(self.root, "clipboard_clear") as clear, patch.object(
        ...
        self.assertIn("  1 NAME John /DOE/", append.call_args[0][0])
        self.assertEqual(
            panel.copy_button.cget("text"), self.viewer.translator.get("ui.copied")
        )

    def test_individual_view_displays_gallery_media_list(self):
        view = self.viewer.individual_view
        media_1 = Mock(pointer="@O1@", title="Portrait", file="portrait.jpg")
        media_2 = Mock(pointer="@O2@", title="Family", file="family.png")
        view.set_object_resolver(
            lambda pointer: {"@O1@": media_1, "@O2@": media_2}.get(pointer)
        )
        view.set_object_display_name_resolver(
            lambda media, entity_type=None: getattr(media, "title", "")
            or getattr(media, "file", "")
        )

        individual = Mock()
        individual.pointer = "@I1@"
        individual.name = "John /DOE/"
        individual.sex = "M"
        individual.birth_date = ""
        individual.birth_place = ""
        individual.baptism_date = ""
        individual.baptism_place = ""
        individual.death_date = ""
        individual.death_place = ""
        individual.death_confirmed = False
        ...
        individual.fams = []
        individual.obje = ["@O1@", "@O2@"]
        individual.events = []

        view.display(individual)
        self.root.update_idletasks()

        gallery_text = [
            child.cget("text")
            for child in view.gallery_container.winfo_children()
            if hasattr(child, "cget") and child.winfo_class() in {"TLabel", "Label"}
        ]
        self.assertIn("Portrait", gallery_text)
        self.assertIn("Family", gallery_text)
        self.assertIn("@O1@", gallery_text)
        self.assertIn("@O2@", gallery_text)
