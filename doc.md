...
La langue de l’interface se choisit dans `Aide > Langue`. L’anglais est utilisé
par défaut pour une nouvelle installation. Le choix est enregistré dans
`~/.gedcom_viewer.json` et prend effet au prochain démarrage. Les langues
disponibles sont l’anglais, le français, l’espagnol, l’allemand, l’italien et le
portugais.
Les catalogues de traduction sont conservés dans `ui/locales/`.
Les traductions encore absentes dans une langue utilisent l’anglais comme repli.
